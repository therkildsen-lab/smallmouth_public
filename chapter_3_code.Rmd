...
# length histos
      fig_length<-
        ...
        theme_cowplot() +
        coord_flip() +
        scale_y_discrete(breaks = c(2000, 2005, 2010, 2015, 2020))

# female gsi age-4
      fig_female_gsi_data<-allFish %>% 
        filter(!is.na(newGSI), sex == 'F', (WATER == 'LML' | WATER == 'WLL'), ageTrue == 4, year < 2021) %>% 
        mutate(Lake = if_else(WATER == 'LML', 'Suppression', 'Reference'),
               Lake = factor(Lake, levels = c('Suppression','Reference')),
      ...
      summary(lm(newGSI ~ year, 
                 data = filter(fig_female_gsi_data, Lake == 'Suppression')))
...
# male gsi age-3
      ...
      summary(lm(newGSI ~ year,
                 data = filter(fig_male_gsi_data, Lake == 'Suppression')))
...
# Figure 1f
fig_catch_age<-mutate(catch_curves_out, Years = if_else(year < 2001, '1998-2000', '2001-2020'),
       suppression_age = paste0(Years, age_catch_curve)) %>% 
  filter(age_catch_curve < 7) %>% 
  ggplot(aes(age_catch_curve, n, color = Years)) + 
...
fig_survival_data<-summ_out %>% 
  filter(mortality == 'S',
         year < 2021) %>% 
  mutate(Years = if_else(year < 2001, '1998-2000', '2001-2020'), 
         Estimate = Estimate/100) 

fig_survival<-fig_survival_data %>% 
  ggplot(aes(Years, Estimate, group = Years, color = Years)) + 
  geom_boxplot() + 
...
paste0('Survival was reduced by ', round(1-calc_diff$survival[2]/calc_diff$survival[1], 2)*100, '%')
...
plot_grid(
    ...
    , plot_grid(fig_length_age +
        ...
        ggsave('/workdir/smallmouth/chapter_3_exports/Fig_1.png', height=10, width = 10)
...
# filter(A_maf, chromo == 'NW_024044237.1_RagTag', between(position, 12740697-100, 12740697+100)) # starts at 0.17
# filter(D_maf, chromo == 'NW_024044237.1_RagTag', position == 201+12740697-1) # ends at 0.91
# 
# # lcwgs chr 7
# filter(ordered_amplicons, microhap_name == 'adaptive_lg7_25601423') 
# filter(A_maf, chromo == 'NW_024040596.1_RagTag', position==25601423)
# filter(D_maf, chromo == 'NW_024040596.1_RagTag', position == 25601423)

lcwgs_af<-tibble(locus = c('adaptive_lg6_3256886-pos201','adaptive_lg19_12740897-pos201','adaptive_lg7_25601423-pos157'), '2000' = c(0.16, 0.17, 0.28), '2019' = c(0.77, 0.91, 0.76)) %>% 
...
loci showing strong AF change, look for significance
```{r}
...
for(loc_in in loci_selected[1:2]){ # exclude lg 7 for now
  ...
  # Build the phenotype dataset
    phenos_dataset<-filter(microhap_snp, locus == loc_in) %>% 
      ...
      mutate(AlleIdx_genoty = as.factor(AlleIdx_genoty))
      ...
      for(sex_in in c('F','M')){
        ...
        # Test if any are different using an anova
          # GSI for each sex independently
            ...
            kt_gsi_sex<-lm(log(newGSI) ~ AlleIdx_genoty, # gsi for each sex
                             data = data_in)

          # length for each sex independently
            data_in<-filter(phenos_dataset,ageLengthed == age_in, sex == sex_in) %>% 
              no_dups(i)
            kt_length_sex<-lm(yearly_length_anomaly ~ AlleIdx_genoty, # yearly length anomaly for each sex
                             data = data_in)
      
          # length for both sexes combined   
            data_in<-filter(phenos_dataset, ageLengthed == age_in) %>% 
              no_dups(i)
            kt_length<-lm(yearly_length_anomaly ~ AlleIdx_genoty, # yearly length anomaly, combining sexes
                             data = data_in)
          ...
          # export regression info
            if(age_in == 3 & sex_in == 'M' & i == 2 & str_detect(loc_in, 'lg6')){
              print('lg 6 age 3 male')
              print(summary(kt_gsi_sex))
              print(length(kt_gsi_sex$residuals))
              print(summary(kt_length_sex))
              print(length(kt_length_sex$residuals))
              length(kt_gsi_sex$residuals)
            }
            
           if(age_in == 3 & sex_in == 'F' & i == 1 & str_detect(loc_in, 'lg19')){
              print('lg 19 age 3 female')
              print(summary(kt_gsi_sex))
              print(length(kt_gsi_sex$residuals))
           }
            
           if(age_in == 3 & i == 1 & str_detect(loc_in, 'lg19')){
              print('lg 19 age 3 both sexes')
              print(summary(kt_length))
              print(length(kt_length$residuals))
           }
            
           if(age_in == 1 & i == 1 & str_detect(loc_in, 'lg19')){
              print('lg 19 age 1 both sexes')
              print(summary(kt_length))
              print(length(kt_length$residuals))
            }
...
for(loc_in in loci_selected){
  ...
  # run analyses over loci
      ...
      for(sex_in in c('F','M')){
        ...
        # Test if any are different using an anova
          # GSI for each sex independently
            ...
            try(kt_gsi_sex<-lm(log(newGSI) ~ idx_genoty, # gsi for each sex
                             data = data_in))
          ...
          # length for each sex independently
            data_in<-filter(phenos_dataset,ageLengthed == age_in, sex == sex_in) %>% 
              no_dups(i)
            kt_length_sex<-try(lm(yearly_length_anomaly ~ idx_genoty, # yearly length anomaly for each sex
                             data = data_in))
          ...
          # length for both sexes combined   
            data_in<-filter(phenos_dataset, ageLengthed == age_in) %>% 
              no_dups(i)
            kt_length<-try(lm(yearly_length_anomaly ~ idx_genoty, # yearly length anomaly, combining sexes
                             data = data_in))
          ...
          # consolidate the ANOVA p-values
            try(tib_out_sex<-bind_rows(tib_out_sex, 
                                    tibble(ageL = age_in, 
                                    sex = sex_in, 
                                    i=i,
                                    gsi_anova_bysex = anova(kt_gsi_sex)$`Pr(>F)`[1], 
    ...
    # length
      len_out[[loc_in]]<-phenos_dataset_filtered %>% 
    ...
    # gsi
      gsi_out[[paste0(loc_in, '_M')]]<-phenos_dataset_sex %>% 
        filter(between(ageLengthed, 1, 4), sex_in == 'Male') %>% 
      ...
      gsi_out[[paste0(loc_in, '_F')]]<-phenos_dataset_sex %>% 
        filter(between(ageLengthed, 1, 4), sex_in == 'Female') %>% 
        ggplot(aes(as.character(ageLengthed), newGSI, color = genoty)) + 
        geom_boxplot(outlier.color = NA) + 
        labs(x='Age', y = 'Gonadal-Somatic\nIndex', color = 'Genotype') + 
        scale_color_viridis_d(option = 'D') + 
...
plot_grid(
  plot_grid(
      ...
      , af_out$`adaptive_lg6_3256886-pos201` + 
        theme(legend.position = 'none') + 
        ...
        theme(axis.title.x=element_blank(),
              axis.text.x = element_blank())
      , af_out$`adaptive_lg7_25601423-pos157` + 
        theme(legend.position = 'none') + 
        scale_y_continuous(limits = c(0,1),
                           breaks = seq(0,1, 0.25)) +
        ylab('Chromosome 7\nallele frequency') +
        ...
        theme(axis.title.x=element_blank(),
              axis.text.x = element_blank())
      , af_out$`adaptive_lg19_12740897-pos201` + 
        theme(legend.position = 'none') + 
        ylab('Chromosome 19\nallele frequency') +
        scale_y_continuous(limits = c(0,1),
                           breaks = seq(0,1, 0.25)) +        
  ...
  , plot_grid(
    ...
    , theme(panel.background = element_blank())
    , get_legend(af_out$`adaptive_lg7_25601423-pos157` + 
                   labs(color = 'Panel'))
    , theme(panel.background = element_blank())
    , get_legend(Ho_out)
    , ggplot() + 
...
ggsave('/workdir/smallmouth/chapter_3_exports/fig2.png', height=20, width = 10)
...
```{r}
plot_grid(
  gsi_out$`adaptive_lg6_3256886-pos201_F` +
    ylab('Female\ngonadal-somatic index')  +
    theme(axis.title.x=element_blank(),
          axis.text.x = element_blank()) +
    ggtitle('a')
  , gsi_out$`adaptive_lg6_3256886-pos201_M` +
    ylab('Male\ngonadal-somatic index') +
    theme(axis.title.x=element_blank(),
          axis.text.x = element_blank()) +
    ggtitle('b')
  , len_out$`adaptive_lg6_3256886-pos201` +
    ggtitle('c')
  , gsi_out$`adaptive_lg19_12740897-pos201_F` +
    ylab('Female\ngonadal-somatic index') +
    theme(axis.title=element_blank(),
          axis.text = element_blank()) +
    ggtitle('d')
  , gsi_out$`adaptive_lg19_12740897-pos201_M` +
    ylab('Male\ngonadal-somatic index') +
    theme(axis.title=element_blank(),
          axis.text = element_blank()) +
    ggtitle('e')
  , len_out$`adaptive_lg19_12740897-pos201` +
    ylab('Length (mm)\nof both sexes') +
    ggtitle('f')
  , ncol = 2 
  , byrow = F
...
ggsave('/workdir/smallmouth/chapter_3_exports/fig3.png', height=10, width = 10)
```
# Supplement
...
## Compare neutral to adaptive AF change annually through the suppression, and make table of sample sizes
...
# Build dataset
    fst_test<-
      microhap_snp %>% 
      left_join(dplyr::select(phenos, GENID_duplicate, WATER, year_all=year)) %>% 
      ...
      filter(pos == min(pos)) %>% 
      ungroup() %>% 
      mutate(loc_type = if_else(str_detect(locus, 'parentage'), 'neutral-',
                                if_else(str_detect(locus, 'lg19'), 'lg19-', 'lg6-')),
             locus = paste0(loc_type,as.integer(as.factor(locus))),
             year = if_else(between(year_all, 1995, 2000), 1998, year_all))# rename loci to work in genind

# Export sample sizes
    fst_test %>% 
      group_by(locus, year_all) %>% 
      tally() %>% 
      filter(year_all<2021) %>% 
      arrange(year_all) %>% 
      mutate(year_all = paste0('`', str_sub(year_all, 3))) %>% 
      pivot_wider(names_from = year_all, values_from = n) %>% 
      replace(is.na(.), 0) %>% 
      separate(locus, into = c('name','name2'), sep = '-', remove = F) %>% 
      mutate(name2 = as.integer(name2)) %>% 
      arrange(name2) %>% 
      dplyr::select(-contains('name')) %>% 
      write_csv('/workdir/smallmouth/chapter_3_exports/af_sample_size.csv')
...
# Calculate annual allele frequency change per locus
    ...
    af_out<-af_out %>% 
      mutate(loc_name = if_else(str_detect(loc, 'neutral'), ' Neutral', 
...
## Neutral vs adaptive AF at the beginning and end of each bottleneck

Compare 1995-2000 vs 2001 and 2007 vs 2013 (because they have the most data). Take the first SNP per microhap
```{r}
# decide on how many individuals per year we want
    min_n<-5
...
# Compare 90's (95-2000) to 2001 and 2006-2011
    af_out<-tibble()
    for(loc_in in unique(fst_test$locus)){
      af_temp<-filter(fst_test, locus == loc_in) %>% 
        group_by(year) %>% 
      ...
      af_in<-af_temp %>% 
        ...
        ungroup() %>% 
        mutate(yeardiff = year - lag(year),
               af_diff_short_lag = abs(af - lag(af)) / yeardiff,
               loc = loc_in)
      
    # Get the difference between 1998 and 2001 and set to NA if either year doesn't exist
      change_1998_2001<-abs(filter(af_in, year == 1998)$af - filter(af_in, year == 2001)$af )
      change_1998_2001<-if(length(change_1998_2001) == 0) NA else change_1998_2001

    # Get the difference between 2007 and 2013 and set to NA if either year doesn't exist
      change_2007_2013<-abs(filter(af_in, year == 2007)$af - filter(af_in, year == 2013)$af )
      change_2007_2013<-if(length(change_2007_2013) == 0) NA else change_2007_2013
      
      af_out<-af_in %>% 
        mutate(`1998-2001`=change_1998_2001,
               `2007-2013`=change_2007_2013) %>% 
        bind_rows(af_out)
    }
    
  # Plot the results
    af_out<-af_out %>% 
      distinct(loc, `1998-2001`, `2007-2013`) %>% 
      pivot_longer(-loc, values_to = 'af_diff') %>% 
      filter(!is.na(af_diff)) %>% 
      mutate(loc_name = if_else(str_detect(loc, 'neutral'), ' Neutral', 
                                if_else(str_detect(loc, '19'), 'Chromsome 19','Chromosome 6')))
    
    af_out  %>% 
      ggplot(aes(name, af_diff, color = loc_name)) +
      geom_boxplot() +
      geom_point(position = position_jitterdodge(jitter.width = 0.05, jitter.height = 0)) +
      labs(x = 'Bottleneck years', y = 'Absolute allele frequency change', color = '') +
      theme_cowplot()
    
  # Test for statistical signiicance
    pv_out<-tibble()
    for(year_grp in unique(af_out$name)){
      
      #year_grp<-"2007-2013"

      dist_in<-af_out %>% 
        filter(name == year_grp)
      
      ggplot(dist_in, aes(af_diff))  + geom_histogram() + geom_point(aes(y=1, color = loc_name))
